...
class usb_20x(mccUSB):
    ...
    DPORT = 0x01  # Read digital port pins / write output_data latch register
    DLATCH = 0x02  # Read/Write digital port output_data latch register
    ...
    AOUT = 0x18  # Read/write Analog output_data channel
    ...
    def AInScanStart(self, count, frequency, channels, options, trigger_source, trigger_mode):
        ...
        output_data at the PACER_OUT pin at every pacer_period interval if SYNC is
        configured as an output_data. The equation for calculating pacer_period
        is:
    ...
    def AInScanClearFIFO(self):
        """
        The command clears the internal scan endpoint FIFOs
        """
        request_type = (HOST_TO_DEVICE | VENDOR_TYPE | DEVICE_RECIPIENT)
...
class usb_202(usb_20x):
    ...
    def AOut(self, channel, value):
        """
        This command writes the values of an analog output_data channel
    ...
    def AOutR(self, channel=0):
        """
        This command reads the value of  both  analog output_data channels
        """
        request_type = (DEVICE_TO_HOST | VENDOR_TYPE | DEVICE_RECIPIENT)
        request = self.AOUT
        wValue = 0
